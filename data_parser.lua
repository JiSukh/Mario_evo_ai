...
function update_near_tiles(tile_x, tile_y)
    ...
    for dx = -1, 7, 1 do
        for dy = -4, 4, 1 do
            local tile_check_x = mario_x + dx
            local tile_check_y = mario_y + dy
...
function draw_mario()
    ...
    local mario_x = emu.read(0x0086, emu.memType.nesMemory, false) -- Local X position within the 'screen'
    local mario_y = emu.read(0x00CE, emu.memType.nesMemory, false) -- Y position

    -- Calculate Mario's global X position in pixels
    local global_mario_x = (screen_index * 256) + mario_x

    -- Convert to tile-based coordinates
    local tile_x = global_mario_x / TILE_SIZE 
    local tile_y = mario_y / TILE_SIZE
